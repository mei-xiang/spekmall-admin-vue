...
<template>
  <div class="tabs" id="tabs">
    ...
    <!-- 全部菜单列表 -->
    <div class="tabs-list" v-show="isShowAllTabsBtn">
      <el-dropdown trigger="click" @command="clickTabsList">
        <div class="tabs-list-btn">
          <i class="el-icon-caret-bottom"></i>
        </div>
        <el-dropdown-menu slot="dropdown">
          <template v-for="(item, index) in options">
            <el-dropdown-item :command="item.route" :key="index">{{item.name}}</el-dropdown-item>
          </template>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
...
export default {
  ...
  methods: {
    ...
    tabClick(tab) {
      ...
      // if (this.$route.path.indexOf('supperApplyInfo') != -1) {
      //   this.$router.push({
      //     path: '/supperApplyInfo',
      //     query: getStore({ name: 'supperApplyInfo' })
      //   })
      // }
    },
    keepPramars() {
      switch (this.$route.path) {
        case '/supperApplyInfo':
          this.$router.push({
            path: '/supperApplyInfo',
            query: getStore({ name: 'supperApplyInfo' })
          })
          break
        case '/supplierMemberInfo':
          this.$router.push({
            path: '/supplierMemberInfo',
            query: getStore({ name: 'supplierMemberInfo' })
          })
          break
        case '/supplierShopInfo':
          this.$router.push({
            path: '/supplierShopInfo',
            query: getStore({ name: 'supplierShopInfo' })
          })
          break
        case '/productListInfo':
          this.$router.push({
            path: '/productListInfo',
            query: getStore({ name: 'productListInfo' })
          })
          break
        case '/selfShopInfo':
          this.$router.push({
            path: '/selfShopInfo',
            query: getStore({ name: 'selfShopInfo' })
          })
          break
        case '/selfProductInfo':
          this.$router.push({
            path: '/selfProductInfo',
            query: getStore({ name: 'selfProductInfo' })
          })
          break
        case '/orderInfo':
          this.$router.push({
            path: '/orderInfo',
            query: getStore({ name: 'orderInfo' })
          })
          break
        case '/orderRefund':
          this.$router.push({
            path: '/orderRefund',
            query: getStore({ name: 'orderRefund' })
          })
          break
        case '/orderBuyInfo':
          this.$router.push({
            path: '/orderBuyInfo',
            query: getStore({ name: 'orderBuyInfo' })
          })
          break
      }
    },
