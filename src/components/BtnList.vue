...
<template>
	<el-button-group class="button-group">
		<template v-for="(item,index) in showBtnData">
			<el-button v-if="!item.hidden && (!item.permission || userInfo.permissions.indexOf(item.permission) !== -1)"
					   size="mini"
					   :key="index"
					   :icon="item.icon"
					   :disabled='item.disabled?item.disabled(): false'
					   @click="clickBtn(item.fn)">{{item.label}}</el-button>
		</template>
		<el-button v-if="btnListData.length>showCount"
				   size="mini"
				   icon="el-icon-more"
				   @click="showAll"></el-button>
		<el-button v-if="showCount>maxShow"
				   size="mini"
				   icon="el-icon-d-arrow-right"
				   @click="showMax"></el-button>
	</el-button-group>
</template>
