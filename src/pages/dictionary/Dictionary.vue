...
export default {
	...
	methods: {
		...
		editBtn() {
			...
			this.axios.get(`/api/dictionary/detail/${this.choicedData[0].id}`).then((res) => {
				let { id, name } = this.leftChoiced;
				this.addOrEditData = {
					id: res.data.id,
					pId: id,
		...
		submit(isAddType) {
			this.$refs.addOrEditForm.validate((data) => {
				let fn = "post";
				let url = "/api/dictionary/detail";
				if (!isAddType) {
					fn = "put";
					url = `/api/dictionary/detail/${data.id}`;
				}
		...
		delBtn() {
			this.$delBtn(() => {
				...
				this.axios.del(`/api/dictionary/detail/${ids}`).then((res) => {
		...
		getList() {
			...
			this.axios.get("/api/dictionary/detail", this.searchParam).then((res) => {
