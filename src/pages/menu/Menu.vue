...
<template>
	<div class="menu">
		...
		<div class="setup-box top clearfix">
			<div class="form-box fl">
				<el-form :inline="true"
					...
					<el-form-item label="是否启用：">
						<el-select v-model="searchData.isEnable"
								   size="mini">
							<el-option label="全部"
			...
			<div class="btn-box fr">
				<btn-list :btnListData="btnListData"
						  ...
						  @close="closeOrOpenTree(false)"></btn-list>
			</div>
		</div>
		<!-- 树形表格区 -->
		<Table-tree ref="tableTree"
					...
					@clickAdd="clickAdd"
					@clickEdit="clickEdit"
					@clickDelete="clickDelete"></Table-tree>
		...
		<el-dialog :title="isAddType?'新增':'修改'"
			...
			<main-form ref="AddOrEditRorm"
					   :rules="rules"
					   :data="chioceData"
					   :form="formConfig"
					   :options="options"
					   @nodeChange="nodeChange"></main-form>
			<div slot="footer">
				...
				<el-button type="primary"
						   size="small"
						   v-else
						   @click="submit(false)">确 定</el-button>
		...
		return {
			...
			headerData: [
				...
				},
				{
					...
					bodyAlign: "center",
					add: (data) => {
						return this.$isPermission("menu")
					},
					edit: (data) => {
						return this.$isPermission("menu")
					},
					delete: (data) => {
						if (!this.$isPermission("menu") || data.children) {
							return false
						} else {
							return true
						}
	...
	methods: {
		...
		getList(val) {
			this.axios.get(`/api/sysmenus`, {
				...
				this.originListData = res.data.records;
		...
		buildTree(data) {
			...
			if (menuName !== '' || isEnable !== null) {
				for (let index = 0; index < newData.length; index++) {
					...
					if (menuName !== '' && item.menuName.toLowerCase().indexOf(menuName.toLowerCase()) === -1) {
						continue;
					}
		...
		setMenuOption(disabledId) {
			...
			if (disabledId) {
				...
				let obj = this.$treeFn.findNodeById(treeCopy, disabledId, { children: "children" });
				obj.disabled = true;
				delete obj.children;
			}
		...
		// 点击修改
		clickEdit(data) {
			this.axios.get(`/api/sysmenus/${data.id}`).then((res) => {
		...
		clickDelete(node, data) {
			this.$delBtn(() => {
				this.axios.del(`/api/sysmenus/${node.id}`).then((res) => {
					this.$message({
						message: "操作成功",
						type: 'success'
					})
		...
		submit(isAddType) {
			...
			if (isAddType) {
				...
				type = 'put';
				url = `/api/sysmenus/${this.chioceData.id}`;
			}
			this.$refs["AddOrEditRorm"].validate((valid) => {
				if (valid) {
					this.axios[type](url, this.chioceData).then((res) => {
						this.$message({
							message: "操作成功",
							type: 'success'
						})
