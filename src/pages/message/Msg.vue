    ...
    <div class="webTalk-panel">
      ...
      <div class="webTalk-panel-left">
        <div class="webTalk-search">
          <el-input type="text" placeholder="搜索" class="input-with-select" v-model="keyWord">
            <el-select v-model="isOnlne" slot="prepend" placeholder="请选择">
          ...
          <div class="webTalk-users-listPanel ui-scroll">
            ...
            >
              <div class="webTalk-user-avatar">
                <el-avatar :size="40" icon="el-icon-user-solid" :src="user.avatar"></el-avatar>
              </div>
              <div class="webTalk-user-InfoBox">
                <div class="webTalk-user-name">
                  【{{user.userType | userType}}】{{user.nickname}}
                  <span
                    class="webTalk-message-time"
                    :class="{'fn-green':user.online,'fn-gray':!user.online}"
                  >{{user.online? '在线': '离线'}}</span>
                </div>
                <div class="webTalk-user-message">
                  {{user.messageTime}}
                  <span
                    class="webTalk-unReaderNum"
                    v-show="user.unReaderNum > 0"
                  >{{user.unReaderNum}}</span>
                </div>
              </div>
            </div>

            <div
              class="webTalk-show-more"
              @click="showMore"
              v-show="leftUserList.length < userList[isOnlne].length && keyWord === ''"
            >查看更多</div>
            <div class="webTalk-no-user" v-show="leftUserList.length === 0">未查到相关数据</div>
          </div>
        </div>
      </div>
      <!-- 右侧信息区 -->
      <div class="webTalk-panel-right">
        <div class="webTalk-message-container">
          <div class="webTalk-title">
            <h3
              class="webTalk-title-text"
              v-show="channel.id"
            >【{{channel.userType | userType}}】{{channel.nickname}}({{channel.online === 1?'在线':'离线'}})</h3>
          </div>
          <div class="webTalk-message-box">
            <div class="webTalk-message-list ui-scroll" ref="messageList">
              ...
              >查看更多</div>
              <div
                class="webTalk-message-item"
                v-for="(message, index) in messageList"
                :key="index"
                :class="message.sendUserId === channel.id?'webTalk-message-receive':'webTalk-message-send'"
              >
                <div class="webTalk-avatarBox">
                  <el-avatar :size="40" icon="el-icon-user-solid" :src="message.avatar"></el-avatar>
                </div>
                <div class="webTalk-message-body">
                  <div class="webTalk-from-info" v-if="message.sendUserId === channel.id">
                  ...
                  <div class="webTalk-message-text" v-html="parserExpression(message.message)"></div>
                </div>
              </div>
            </div>
          </div>
          <div class="webTalk-footer">
            ...
            <div class="webTalk-message-inputBox">
              <textarea
                class="webTalk-message-input ui-scroll"
                v-model="text"
                placeholder="请输入要说的话..."
                :maxlength="200"
                @focus="isShow = false"
      ...
      if (typeof text !== 'undefined') {
        ...
        if (sArr && sArr.length > 0) {
          for (let i = 0; i < sArr.length; i++) {
            ...
            if (index !== -1) {
              const reStr =
                '<img src=' + arr[index].url + " height='20' width='20' />"
