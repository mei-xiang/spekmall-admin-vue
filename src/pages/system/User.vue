...
<template>
  <div class="user">
    <main-page ref="mainPage"
               ...
               @switchChange="switchChange"></main-page>
    ...
    <el-dialog :title="dialogType === 0?'新增':'修改'"
      ...
      <span v-show="dialogType !== 2"
            class="label">头像：</span>
      <el-upload v-show="dialogType !== 2"
...
export default {
  ...
  data() {
    return {
      ...
      addOrEditRules: {
        ...
        roleId: [
          this.$rules.setRequired("请选择角色", "change")
        ],
        phone: [
          { required: false, validator: this.$rules.checkPhone, trigger: 'blur' }
        ],
  ...
  methods: {
    ...
    switchChange(val, index) {
      this.axios.put("/api/users/enable", {
        id: val.id,
    ...
    getDataById(data) {
      return new Promise((resolve, reject) => {
        this.axios.get(`/api/users/${data.id}`).then((res) => {
          resolve(res)
        }).catch((err) => {
          reject(err)
        });
