...
export default {
	...
	methods: {
		...
		refresh() {
			this.axios.get(`/dictionary/page`, {
				pageNum: 1,
				size: 99999
			}).then((res) => {
		...
		// 修改部分 -----------------------------
		clickEdit(data, node) {
			this.axios.get(`/dictionary/${data.id}`).then((res) => {
		...
		clickDelete(data, node) {
			this.$delBtn(() => {
				this.axios.del(`/dictionary/${data.id}`, {
					id: data.id
				}).then((res) => {
					this.$message({
						message: "删除成功",
